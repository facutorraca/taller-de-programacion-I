...
int client_start_to_send(client_t* client, const char* inst, int len_inst,int (*send_control)(const char*, int)) {
    int bytes_sent = 0, total_bytes = 0;
    do {
        bytes_sent = socket_send(&client->c_socket, inst, len_inst);
        total_bytes = bytes_sent + total_bytes;
    } while (send_control(inst, total_bytes) == ERROR);
    return SUCCESS;
}
